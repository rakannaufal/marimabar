...
import { configured, demoMode, supabase } from '../lib/supabase'
...
async function load() {
  if (!userId.value || (!configured && !demoMode)) { loading.value = false; return }
...
async function saveGame() {
  ...
  busy.value = true; error.value = ''; notice.value = ''
  try {
    const input = { ign:ign.value.trim(), game_id_private:privateId.value.trim() || null, primary_rank_option_id:rank.value || null, primary_rank_mode_option_id:rankMode.value || null, primary_role_option_id:role.value || null, region_option_id:region.value || null, visibility:gameVisibility.value, attributes:{} }
...
<template>
  <main class="account page-shell"><nav class="breadcrumb"><RouterLink to="/">Beranda</RouterLink><span>/</span><strong>Profil Saya</strong></nav><header class="page-heading"><div><h1>Edit profil &amp; akun</h1><p>Atur identitas publik, ketersediaan manual, dan profil game kamu.</p></div></header>
    <p v-if="!configured && !demoMode" role="alert" class="error">Layanan belum dikonfigurasi.</p><p v-else-if="!userId">Masuk untuk mengelola profil. <RouterLink to="/login">Masuk</RouterLink></p>
    <template v-else><nav class="section-tabs" aria-label="Bagian profil"><button type="button" :aria-current="tab === 'info' ? 'page' : undefined" @click="tab = 'info'">Info Umum</button><button type="button" :aria-current="tab === 'games' ? 'page' : undefined" @click="tab = 'games'">Profil Game <span class="count">{{ profiles.length }}</span></button><button type="button" :aria-current="tab === 'settings' ? 'page' : undefined" @click="tab = 'settings'">Pengaturan Akun</button></nav>
      ...
      <section v-if="!loading && tab === 'settings'" class="panel"><h2>Pengaturan akun</h2><form v-if="configured && !demoMode" @submit.prevent="savePassword"><h3>Ganti password</h3><label>Password baru<input v-model="newPassword" type="password" minlength="8" autocomplete="new-password" required /></label><label>Konfirmasi password baru<input v-model="confirmPassword" type="password" minlength="8" autocomplete="new-password" required /></label><button type="submit" :disabled="busy">Perbarui password</button></form><p v-else class="muted">Password tidak digunakan di mode demo.</p><div class="danger-zone"><h3>Hapus akun</h3><p>Penghapusan akun belum tersedia di aplikasi ini. Jangan anggap data terhapus sebelum permintaan diproses oleh pengelola layanan.</p></div></section>
    </template>
  </main>
</template>
<style scoped>
