...
function openReport() { reportSent.value = false; reportOpen.value = true; inviteOpen.value = false; actionError.value = ''; notice.value = ''; category.value = ''; description.value = '' }
...
<template>
  <main class="page-shell profile-page">
    ...
    <template v-else>
      ...
      <div class="profile-layout"><div class="profile-main"><section class="content-card"><h2>Tentang pemain</h2><p v-if="player.bio" class="profile-bio">{{ player.bio }}</p><p v-else class="muted">Pemain ini belum menambahkan bio.</p></section><section class="content-card"><h2>Profil game</h2><h3>{{ player.game_name }}</h3><div class="chip-list"><span v-if="player.rank" class="chip chip--violet">{{ player.rank }}</span><span v-if="player.role" class="chip">{{ player.role }}</span><span v-if="player.region" class="chip">{{ player.region }}</span></div><p v-if="updated" class="fine-print">Diperbarui {{ updated }}</p><p v-if="stale" class="fine-print">Profil ini perlu diperbarui.</p><p class="fine-print">Data diisi pengguna, belum terverifikasi. ID game hanya dibagikan oleh pemilik lewat chat setelah ajakan diterima.</p></section></div><aside class="profile-aside"><section class="content-card"><h2>Info singkat</h2><p class="muted">{{ player.ready ? 'Sedang mencari mabar' : 'Belum siap mabar' }}</p><p v-if="player.region">Region pilihan: {{ player.region }}</p><p class="fine-print">Jam aktif dan statistik mabar belum tersedia.</p></section><section class="content-card safety-card"><h2>Jaga ruang mabar</h2><p class="muted">Ada profil yang mengganggu? Laporkan atau blokir pemain ini.</p><div class="safety-actions"><button class="text-link text-link--danger" type="button" @click="openReport">Laporkan profil</button><button class="text-link" type="button" :disabled="busy" @click="confirmBlock">Blokir pemain</button></div></section></aside></div>
      <div v-if="reportOpen" class="report-overlay" @click.self="reportOpen = false"><section class="report-sheet" role="dialog" aria-modal="true" aria-labelledby="report-title" @keydown.esc="reportOpen = false"><button type="button" class="report-close" aria-label="Tutup laporan" @click="reportOpen = false">Tutup</button><template v-if="reportSent"><h2 id="report-title">Laporan diterima</h2><p>{{ notice }}</p><p class="muted">Tim moderasi akan meninjau laporan sesuai antrean. Tidak ada waktu penyelesaian yang dijanjikan.</p><button class="button button--primary" type="button" @click="reportOpen = false">Selesai</button></template><form v-else @submit.prevent="submitReport"><h2 id="report-title">Kenapa kamu melaporkan profil ini?</h2><p class="muted">Bantu jaga lingkungan mabar tetap nyaman.</p><fieldset class="reason-list"><legend>Alasan laporan</legend><label v-for="option in categories" :key="option.code"><input v-model="category" type="radio" name="category" :value="option.code" required />{{ option.label }}</label></fieldset><label class="field">Detail laporan<textarea v-model="description" minlength="10" maxlength="2000" required rows="4" placeholder="Ceritakan kejadian, minimal 10 karakter."></textarea></label><p v-if="actionError" class="inline-error" role="alert">{{ actionError }}</p><div class="action-row"><button class="button button--outline" type="button" @click="reportOpen = false">Batal</button><button class="button button--primary" type="submit" :disabled="busy || !category || description.trim().length < 10">{{ busy ? 'Mengirim…' : 'Kirim laporan' }}</button></div></form></section></div>
    </template>
  </main>
</template>
<style scoped>
